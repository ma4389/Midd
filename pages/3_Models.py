...
st.markdown("Model: The specific car model (e.g., Mustang, Soul, Silverado)")
...
# Group by 'model' and selected interest
if interest in df.columns:
    if interest == 'year':
        grouped_df = df.groupby('model')[interest].agg(lambda x: x.mode()[0]).reset_index()
    else:
        grouped_df = df.groupby('model')[interest].mean().reset_index()
    ...
    st.plotly_chart(px.bar(top_10_df, x='model', y=interest, title=f'Top 10 {interest}', color='model'), use_container_width=True)

    # Histogram (count plot) for 'model'
    st.header("Histogram of Model")
    fig_hist = px.histogram(df, x='model', title='Distribution of Car Models', color='model')
    st.plotly_chart(fig_hist, use_container_width=True)

    # Line chart
    st.header(f"Line Chart of Model vs. {interest}")
    fig_line = px.line(grouped_df, x='model', y=interest, title=f'Line Chart of Model vs. {interest}')
    st.plotly_chart(fig_line, use_container_width=True)

    # Bar chart
    st.header(f"Bar Chart of Model vs. {interest}")
    fig_bar = px.bar(grouped_df, x='model', y=interest, title=f'Bar Chart of Model vs. {interest}', color='model')
    st.plotly_chart(fig_bar, use_container_width=True)

    # Pie Chart (assuming categorical Y-axis)
    if df[interest].dtype == "object":
        st.header(f"Distribution of Cars by {interest}")
        fig_pie = px.pie(grouped_df, names='model', values=interest, title=f'Distribution of Cars by {interest}', color='model')
