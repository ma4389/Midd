...
if has_models_column:
    ...
    st.plotly_chart(px.bar(mydf, x="trim", y=interest, title=f"Top 10 {interest}" , color = 'trim'), use_container_width=True)
else:
    st.write("The 'trim' column is not available in the dataset.")
st.header(f"Line scatter of {interest}")
fig_scatter = px.scatter(df, x='trim', y=interest , color = 'trim')
